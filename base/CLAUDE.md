...
- **Offer explicit execution choices for non-trivial work.** Let the user choose **minimal** or **complete**, then **module-by-module** or **task-by-task** execution.
...
- **Update docs immediately.** If you change behavior, API, architecture, or workflow expectations, update the corresponding doc in the same batch — not later.
...
- **Commit in batches.** Keep each reviewed batch small, independently verifiable, and ready for its own commit.

## Context Management

- If the task involves multiple files, read all relevant files before making changes.
- After each completed batch, update `docs/plan.md` to reflect current state.
...
- **Web search:** Start with repository files and local docs. If external facts are still uncertain, let the user choose local-only analysis, search only when needed, or an already configured search tool. Do not guess and do not search speculatively.
...
- Do not defer required doc updates or collapse multiple concerns into one large commit.
